...
- 4 processing nodes: **Extract, Search, Validate, Merge**
...
- **Extract** reads the incident from PostgreSQL.
...
- **Merge** uses an LLM to extract structured fields from confirmed articles.
...
A pilot study on 10 records identified 4 systematic failure modes: aggregation source contamination, merge normalization bugs, genuinely conflicting accounts, and identity ambiguity. After targeted fixes and regression tests, a holdout evaluation ran on 100 records stratified by incident year (2014–2024) with strict dev/holdout separation.
...
The merge node originally used an alias dictionary (`{"African American": "Black", "Caucasian": "White", ...}`) that couldn't handle variations like "Hispanic/Latino male." The fix was keyword-based matching that strips gender words first, then maps to canonical categories using regex word boundaries. The lesson here is that a 30% accuracy improvement came from a string matching fix that only surfaced during evaluation.
...
I submitted this work to the SciPy 2026 conference, which is currently under review. Major planned work includes batch processing across all remaining records with priority ordering, evaluation on the officers-shot dataset, and a human review UI.
Some improvements also include tightening the extraction prompt to fix time-of-day parse errors, adding an outcome-only completion guard that requires at least two extracted fields. 

But other improvements need domain expertise from TJI volunteers and organizers. The weapon category map, for instance, currently misses mappings like sawed-off shotgun → shotgun and machete → knife because the government reporting forms use a fixed set of categories that don't always match how news articles describe weapons. Expanding that map correctly requires someone who understands both the reporting conventions and the real-world ambiguities. For instance, a BB gun and a pellet gun might look the same in a news article but may carry different legal weight. Similar judgment calls will come up as the system processes more records and encounters edge cases in race categorization, location naming, and outcome classification. This is collaborative work, not just engineering work.
